...
export const WEIGHT_MATRIX: Record<string, { A: number[]; B: number[] }> = Object.fromEntries(
...
function computeConfidence(preferences: TuningPreference[], featureScores: FeatureScores): ConfidenceResult {
  ...
  for (const p of preferences) {
    const existing = prefMap.get(p.scenario);
    if (existing && existing !== p.choice && p.choice !== 'NOT_SURE' && p.choice !== 'DISLIKE_BOTH' && existing !== 'NOT_SURE' && existing !== 'DISLIKE_BOTH') {
      intraContradictions++;
    }
    prefMap.set(p.scenario, p.choice);
  }

  if (intraContradictions > 0) {
    contradictionMessages.push(`Detected ${intraContradictions} intra-scenario contradiction(s) (flip-flopping)`);
  }
  ...
  const contradictionPenalty = (contradictionMessages.length * 0.12) + (intraContradictions * 0.1);
...
// ─── FIX #5: Curve Shape Preservation ──────────────────────────────────────────
// OLD: Element-wise tanh saturated dominant bands, flattening the intended EQ shape.
//      RMS normalization didn't stop single-band peaks (sqrt(10)*6 ~ 19dB) from clipping.
// FIX: Implement global peak-scaling to preserve relative ratios perfectly.
const MAX_PEAK_DB = 10.0; // Keep within linear-ish region

function normalizeGainEnergy(gains: number[]): number[] {
  const maxAbs = Math.max(...gains.map(Math.abs));
  if (maxAbs < 0.01) return gains;
  const targetMax = 8; // tanh expressive range
  const scale = Math.min(1, targetMax / maxAbs); // chỉ scale down, không scale up
...
export async function analyzePreferences(
  ...
  const scenarioChoices = new Map(collapsed.map(p => [p.scenario, p.choice]));
  ...
  sigmoidGains = sigmoidGains.map((g, i) => g + bestProfile.bias[i] * (1 - biasStrength));
...
export function pickNextScenario(
  ...
  try {
    ...
    // Bayesian selection failed, falling back to greedy
    return allScenarios.find(s => !usedScenarios.has(s)) || allScenarios[0];
  }
}
